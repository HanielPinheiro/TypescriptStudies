...
customersRoutes.post('/auth/register', async (req, res) => {
    ...
    if (existing)
        throw new AppError({ statusCode: 409, code: 'EMAIL_IN_USE', message: 'E-mail já cadastrado' });
    const passwordHash = await bcrypt.hash(body.password, 10);
    const customer = await createCustomer({ name: body.name, email: body.email.toLowerCase(), passwordHash });
    const accessToken = signAccessToken({ customerId: customer.id });
    res.status(201).json({ customer: { id: customer.id, name: customer.name, email: customer.email }, accessToken });
});
customersRoutes.post('/auth/login', async (req, res) => {
    ...
    if (!ok)
        throw new AppError({ statusCode: 401, code: 'INVALID_CREDENTIALS', message: 'Credenciais inválidas' });
    const accessToken = signAccessToken({ customerId: customer.id });
    res.json({ customer: { id: customer.id, name: customer.name, email: customer.email }, accessToken });
});
customersRoutes.get('/me', requireAuth, async (req, res) => {
    const auth = req.auth;
    const customer = await findCustomerById({ id: auth.customerId });
    if (!customer)
        throw new AppError({ statusCode: 401, code: 'UNAUTHORIZED', message: 'Usuário inválido' });
