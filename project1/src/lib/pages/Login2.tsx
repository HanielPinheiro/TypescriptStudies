...
const Login = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen font-sans">
      ...
      <button
        onClick={toggleTheme}
        className="absolute top-4 right-4 text-xs px-2 py-1 border rounded bg-white dark:bg-gray-900"
      >
        Switch to {theme === "light" ? "Dark" : "Light"} Mode
      </button>

      <section className="flex flex-col md:flex-row h-[60vh] bg-gradient-to-br from-yellow-300 to-yellow-600">
        {/* Left side image (optional extra styling) */}
        <div className="md:w-1/2 h-64 md:h-screen imgLogin flex items-center justify-center">
          <img
            src="/assets/login/MaisGostei.jpeg"
            alt="background"
            className="w-full h-full object-cover hidden md:block opacity-0"
          />
        </div>

        {/* Right side login card */}
        <div className="md:w-1/2 flex items-center justify-center bg-transparent">
          <div className="card bg-white/60 dark:bg-black/40 backdrop-blur-md p-8 rounded-lg shadow-lg w-full max-w-md">
